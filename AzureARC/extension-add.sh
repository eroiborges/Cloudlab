...
tags="env=prod dept=sql" # Update with your desired tags using space-separated key=value pairs
sqlparameter='--settings "{\"SqlManagement\":{\"IsEnabled\":true}, \"LicenseType\":\"<License Type>\", \"ExcludedSqlInstances\":[]}"'
  ...
  if [ -n "$check" ]; then
    ...
    case $extension_name in
      "WindowsAgent.SqlServer")
        az connectedmachine extension create --resource-group $resource_group --machine-name $server --name $extension_name --publisher $Publisher --type $extension_name --type-handler-version $Target_Version --tags $tags $sqlparameter --no-wait true
        ;;
      *)
